...
| T54 | . | FE+API: provider filter on `/sites`. Backend — `SitesViewSet.get_queryset` reads `getlist("provider")` and ORs `Exists(MemberLink.objects.filter(member=OuterRef("pk"), provider__in=selected))` with `~Exists(...)` when the `Tanpa Link` sentinel is present, composed alongside `apply_status_filter` + `?network=`; new `@action(detail=False)` `providers/` returns sorted distinct non-empty providers over `member_queryset_for(request.user)`. FE — multi-select in the toolbar beside the search box, `Select` from `components/ui`, options fetched once from `/providers/`; selection held in state and written as repeated `provider` params; changing it resets `page` to 1 exactly like the search box does. No model change, no migration, no new dependency, no write path | C41,V56 |
