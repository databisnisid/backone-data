    ...
    try:
        ...
        try:
            response = requests.get(
                domain_api + "/api/members/get_by_net_mqtt/" + network_id + "/"
            )
            response_json = response.json()

        except requests.exceptions.HTTPError as e:
            logger.error("HTTP error fetching members for %s: %s", network_id, e.response.text)
...
def get_members_all(domain_api):
    ...
    for network in networks:
        logger.info("NetworkID: %s", network.network_id)
